...
export function airChangesPerHour(tightness: Tightness, floorArea: number, fireplaces: number = 0): number {
  return floorAreaAirChanges(tightness, floorArea) + fireplaceAirChanges(tightness, fireplaces);
}
...
function fireplaceAirChanges(tightness: Tightness, fireplaces: number): number {
  ...
  const airChangesPerFireplace = airChangesPerFireplaceByTightness[tightness] ?? 0;
  return airChangesPerFireplace * fireplaces;
}

export function infiltrationCfm(airChangesPerHour: number, totalVolume: number): number {
  return airChangesPerHour * totalVolume * 0.0167;
}

export function infiltrationHeatLoss(infiltrationCfm: number, temperatureDifference: number): number {
