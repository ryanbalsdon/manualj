import type { FinalValue, KeyExtractor } from "./mapBuilder";
...
describe("createMemoizedMapBuilder", () => {
  ...
  it("should build a memoized nested map correctly", () => {
    ...
    expect(valueAX).toEqual({
      uFactor: 0.1,
      htmByTemperature: { 10: 1, 20: 2 },
    });

    const valueAY = mapA.get("Y");
    expect(valueAY).toEqual({
      uFactor: 0.2,
      htmByTemperature: { 10: 3, 20: 4 },
    });

    const valueBX = mapB.get("X");
    expect(valueBX).toEqual({
      uFactor: 0.3,
      htmByTemperature: { 10: 5, 20: 6 },
    });
  });
  ...
  it("should handle one key level + final value mapping", () => {
    ...
    const buildOneLevelMap = createMemoizedMapBuilder(
      oneLevelData,
      oneLevelKeyExtractors,
    );
    const oneLevelMap = buildOneLevelMap();

    expect(oneLevelMap).toBeInstanceOf(Map);
    expect(oneLevelMap.size).toBe(2);
    expect(oneLevelMap.get("Item1")).toEqual({
      uFactor: 1.0,
      htmByTemperature: { 1: 10 },
    });
    expect(oneLevelMap.get("Item2")).toEqual({
      uFactor: 2.0,
      htmByTemperature: { 2: 20 },
    });
  });
  it("should throw an error if K is an empty array (keyExtractors.length < 2)", () => {
    ...
    expect(() =>
      createMemoizedMapBuilder(simpleData, simpleKeyExtractors),
    ).toThrow(
